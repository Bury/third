...
<!--是-->
      <el-form :inline="true" class="demo-form-inline" :model="ruleForm" :rules="rules" ref="ruleForm" size="mini" v-show="yesFilter">
    ...
    <el-row>
      <el-col>
        <button class="btn" @click="add()">合并数据</button>
      ...
      <template slot-scope="scope" >
        <div style="display: flex;align-items: center" v-show="scope.row.score >= getBackCheck">
          ...
          <div>
            ...
            <span>{{scope.row.customer_created_at | date(4)}}</span>
          </div>
        </div>
        <div style="display: flex;align-items: center" v-show="scope.row.score < getBackCheck">
          <p>/</p>
      ...
      <template slot-scope="scope" >
        <div style="display: flex;flex-direction: column">
          <el-button
            size="mini"
            type="text"
            @click="manage(scope.row)">查看百度人脸图片</el-button>
          <el-button
            size="mini"
            type="text"
            @click="handleEdit(scope.row)">编辑合并数据记录</el-button>
          <el-button
            size="mini"
            type="text"
            @click="handleDelete(scope.row)">查看到店记录</el-button>
          <el-button
            size="mini"
            type="text"
    ...
    <!--查看百度人脸图-->
    <el-dialog title="百度人脸图" :visible.sync="FaceVisible">
      <el-form  label-width="180px" class="demo-ruleForm">
        <div class="shouIm">
          <div v-for="item of getBDImg" class="iamgStyle">
            <img :src="item.avatar" alt="" style="width: 6rem;height: 6rem">
            <p>{{item.ctime}}</p>
          </div>
        </div>
      </el-form>
...
<script src="@/assets/js/dataCollect/dataList.js"></script>
